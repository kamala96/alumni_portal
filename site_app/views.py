...
def reset_failed_login_attempts(profile):
    profile.failed_login_attempts = 0
    profile.lockout_until = None
    profile.save()


def increment_failed_login_attempts(profile):
    profile.failed_login_attempts += 1
    if profile.failed_login_attempts >= 3:
        profile.lockout_until = timezone.now() + timezone.timedelta(hours=24)
    profile.save()
...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None:
            user_profile = AlumniProfile.objects.filter(user=user).first()
            if user_profile:
                if user_profile.lockout_until is None or user_profile.lockout_until <= timezone.now():
                    login(request, user)
                    # token, created = Token.objects.get_or_create(user=user)
                    reset_failed_login_attempts(user_profile)
                    ...
                    messages.error(
                        request, "Your account has been temporarily locked. Please try again later.")
                    return redirect(reverse('handle_nav_menu_click', args=['login']))
            else:
                messages.warning(
                    request, "User exists but has no profile. Please contact admin.")
                return redirect(reverse('handle_nav_menu_click', args=['login']))
        else:
            user_profile = AlumniProfile.objects.filter(
                user__username=username).first()
            if user_profile:
                increment_failed_login_attempts(user_profile)
            messages.warning(
...
def index(request):
    ...
    context = {
        ...
        # [:6]
        'events': events.filter(is_published=True, is_published_on_slider=True).order_by('-created_at'),
...
def handle_nav_menu_click(request, menu_slug):
    ...
    jobs = Paginator(JobPosting.objects.filter(
        is_active=True).order_by('-created_at'), 9)
    ...
    context = {
        ...
        'about_us': about_us,
        # about
        'committees': committees.filter(is_active=True).order_by('order_id')[:8],
        # [:5], committee garally
        'all_committees': committees.filter(is_active=True).order_by('order_id'),
        # [:5],
        'alumni_speechs': alumni_speechs.filter(is_published=True).order_by('-order_id'),
...
def user_create_account(request):
    if request.method == 'POST':
        ...
        if AlumniProfile.objects.filter(phone=phone_number).exists():
            messages.error(
                request, 'Phone number already registered. Please use a different phone number.')
            return redirect(reverse('handle_nav_menu_click', args=['register']))
        ...
        try:
            with transaction.atomic():
                ...
                login(request, user)
                messages.success(
                    request, "User account registered successfully!")
                return redirect(reverse('handle_nav_menu_click', args=['profile']))
...
def alumni_update_profile(request):
    ...
    if request.method == 'POST':
        ...
        if profile.is_sonit_leader:
            profile.sonit_leader_position = request.POST.get(
                'sonit_leader_position')
        else:
...
def alumni_update_password(request):
    if request.method == 'POST':
        ...
        if check_password(old_password, user.password):
            ...
            messages.error(
                request, 'Invalid old password. Password not updated.')
...
def usre_profile_image(request):
    ...
    if request.method == 'POST':
        ...
        if profile_image:
            ...
            alumni_profile.profile_picture.save(
                profile_image.name, ContentFile(temp_file.getvalue()), save=False)
            alumni_profile.save()
        elif cover_image:
            # Open the uploaded image using Pillow
            ...
            temp_file = BytesIO()
            image.save(temp_file, format='PNG')
            alumni_profile.cover_profile.save(
                cover_image.name, ContentFile(temp_file.getvalue()), save=False)
            alumni_profile.save()
...
def add_subscribe(request):
    if request.method == 'POST':
        ...
        if Subscriber.objects.filter(email=email).exists():
            ...
            messages.success(
                request, 'You have successfully subscribed to news notifications.')
...
def notify_subscribers(news):
    ...
    for subscriber in subscribers:
        ...
        html_message = render_to_string(
            'email/email_main.html', {'news': news})
